...
public class CustomerCardInfoManagerImpl extends EntityManagerImpl<CustomerCardInfo, CustomerCardInfoDao> implements
	...
	@Override
	public void cancelLost(Long ccid, boolean flag) {
		try {
			CustomerCardInfo cci = customerCardInfoDao.load(ccid);
			// 1.改变CCI的状态
			cci.setStatus(CustomerCardInfo.STATUS_NORMAL);
			...
			CardInfo card = cci.getCard();
			card.setStatus(CardInfo.STATUS_ENABLE);
			// 从黑名单中移除
			CardBlackList cardBlackList = new CardBlackList();
			cardBlackList.setCustomerCardInfo(cci);
			...
			if (flag) {
				...
				DesiredOperation lockSdDo = desiredOperationManager.getByAidAndProcedureNameAndCustomerCardThatNotExcuted(csd.getSd()
						.getAid(), LocalTransaction.Operation.LOCK_CARD.name(), cci);
				if (null != lockSdDo) {
					desiredOperationManager.remove(lockSdDo);
				}
			}
		} catch (PlatformException pe) {
			throw pe;
