	...
	public boolean handlePublishedApply(SecurityDomainApply apply, boolean result, Map<String, Object> params) throws PlatformException {
		...
		try {
			if (result) {
				...
				//生成密钥
				HsmkeyConfig hsmkeyConfig = null;
				List<HsmkeyConfig> hsmKeyConfigs = null;

				KeyProfile enc = new KeyProfile(sd, KeyProfile.INDEX_ENC, apply.getKeyProfileENC());
				enc.setType(KeyProfile.SCP02_ENC_TYPE);
				enc.setStatus(KeyProfile.STATUS_AVALIABLE);
				
				hsmkeyConfig = new HsmkeyConfig();
				hsmkeyConfig.setIndex((Integer)params.get("indexENC"));
				hsmkeyConfig.setVersion((Integer)params.get("versionENC"));
				hsmkeyConfigManager.saveOrUpdate(hsmkeyConfig);
				
				hsmKeyConfigs = new ArrayList<HsmkeyConfig>(1);
				hsmKeyConfigs.add(hsmkeyConfig);
				enc.setHsmKeyConfigs(hsmKeyConfigs);
				keyProfileManager.saveOrUpdate(enc);
				
				KeyProfile mac = new KeyProfile(sd, KeyProfile.INDEX_MAC, apply.getKeyProfileMAC());
				mac.setType(KeyProfile.SCP02_MAC_TYPE);
				mac.setStatus(KeyProfile.STATUS_AVALIABLE);
				hsmkeyConfig = new HsmkeyConfig();
				hsmkeyConfig.setIndex((Integer)params.get("indexMAC"));
				hsmkeyConfig.setVersion((Integer)params.get("versionMAC"));
				hsmkeyConfigManager.saveOrUpdate(hsmkeyConfig);
				
				hsmKeyConfigs = new ArrayList<HsmkeyConfig>(1);
				hsmKeyConfigs.add(hsmkeyConfig);
				mac.setHsmKeyConfigs(hsmKeyConfigs);
				keyProfileManager.saveOrUpdate(mac);
				
				KeyProfile dek = new KeyProfile(sd, KeyProfile.INDEX_DEK, apply.getKeyProfileDEK());
				dek.setType(KeyProfile.SCP02_DEK_TYPE);
				dek.setStatus(KeyProfile.STATUS_AVALIABLE);
				hsmkeyConfig = new HsmkeyConfig();
				hsmkeyConfig.setIndex((Integer)params.get("indexDEK"));
				hsmkeyConfig.setVersion((Integer)params.get("versionDEK"));
				hsmkeyConfigManager.saveOrUpdate(hsmkeyConfig);
				
				hsmKeyConfigs = new ArrayList<HsmkeyConfig>(1);
				hsmKeyConfigs.add(hsmkeyConfig);
				dek.setHsmKeyConfigs(hsmKeyConfigs);
				keyProfileManager.saveOrUpdate(dek);
