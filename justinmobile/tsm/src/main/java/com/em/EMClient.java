...
public class EMClient {
	...
	/** 用指定的密钥经过若干级分散（可选）和过程密钥计算（可选）后，使用指定的算法和填充方式对
	 *  输入数据进行加密操作。
	 * @param KeyIndex 密钥索引
	 * @param AlgFlag 算法标识：AES，0x88;3DES-ECB，0x81； 3DES-CBC， 0x82；DES-CBC：0x84
	 * @param PadFlag 填充标识：0，外部填充；1，内部填充
	 * @param DivNum 离散次数
	 * @param DivData 离散数据
	 * @param SessionKeyFlag 过程密钥标识：0，无过程密钥；1，有过程密钥
	 * @param SkeySeed 过程密钥
	 * @param DataLen 明文数据长度
	 * @param Data 明文数据
	 * @param CipheredDataLen 密文数据长度
	 * @param CipheredData  密文
	 * @return
	 * @throws Exception
	 */
	 ...
	 * @param SessionKeyFlag 过程密钥标识：0，无过程密钥；1，有过程密钥
	 * @param SkeySeed 过程密钥
	 * @param DataLen 密文数据长度
	 * @param Data 密文数据
	 * @param CipheredDataLen 明文数据长度
	 * @param CipheredData  明文
	 * @return
	 * @throws Exception
	 */
	 ...
	 * @param Key1Index 根密钥1索引号
	 * @param AlgFlag 算法标识：AES，0x88;3DES-ECB，0x81； 3DES-CBC， 0x82；DES-CBC：0x84
	 * @param Pad1Flag 填充标识：0，外部填充；1，内部填充
	 * @param Key1DivNum 根密钥1离散次数
	 * @param Key1DivData 根密钥1离散数据
	 * @param SessionKey1Flag 过程标识：0，无过程密钥；1，有过程密钥
	 * @param Skey1Seed 过程密钥
	 * @param Key2Index	根密钥2索引号
	 * @param Key2DivNum 根密钥2离散次数
	 * @param Key2DivData 根密钥2离散数据
	 * @param Session2KeyFlag 过程标识：0，无过程密钥；1，有过程密钥
	 * @param Skey2Seed 过程密钥
	 * @param inDataLen 根密钥1加密的数据密文长度
	 * @param bInDataByKey1 根密钥1加密的数据密文
	 * @param OutDataLen 根密钥2加密的数据密文长度
	 * @param bOutDataByKey2 根密钥2加密的数据密文
	 * @return
	 * @throws Exception
	 */
	...
	public synchronized int HsmGenerateMulKeyAndCheck(
			int AlgFlag,
			int SessionKeyFlag,// 0：不使用过程密钥 1：使用过程密钥
	...
	private EMResult HSM_GI(String encyFlag, String padMode, String lmkType,
		...
		this.CheckValue(lenOfDesKeyByPK, 1, 9999,
				"lenOfDesKeyByPK数据长度为[1-9999]的整数");
	...
	private String CheckMessHeadAndResult(String recv, String MessHead,
		...
		if (MessHead.equals(MessHeadDest) == false) {
			logger.error("接收数据的消息头不一致.收到的MessHead[" + MessHead
					+ " 期望的MessHeadDest:[" + MessHeadDest + "]");
			return null;
		} else
