...
JIM.CardDriver = new Class(
		{
			...
			options : {
				commonType : 2,
				url : location.protocol + "//" + location.host + ctx
						+ "/html/mocam/?m=handler",
				ctl : null,
				operations : [],
				showMsg : true
			},
			...
			initialize : function(options, ctl) {
				...
				this.options.operations.each(function(item) {
					...
					this.addAppOperate(item.aid, item.operation, appVersion,
							originalCardNo);
				}.bind(this));
			...
			execCallback : function(json) {
				...
				if (response.status.isSuccess()) {
					...
					if (0 == apdusLength) {
						...
						if ($chk(response.progress)
								&& $chk(response.progressPercent)) {
						...
						cardPOR.options.lastData = cardPOR.options.lastData
								+ cardPOR.options.lastAPDUSW;
					...
					this.modal = new LightFace(
							{
								...
								height : 100,
								width : 250,
								content : $('#cardDriverProgressTemplate$')
										.get('html'),
								resetOnScroll : false
							});
				}
...
JIM.CardDriver.ResExecAPDU = new Class(
		{
			...
			initialize : function(options) {
				...
				if ($chk(response.CurrentAppAid)
						&& $chk(response.CurrentAppAid.value)) {
				...
				if ($chk(response.ProgressPercent)
						&& $chk(response.ProgressPercent.value)) {
...
JIM.CardDriver.ReqExecAPDU = new Class(
		{
			...
			toJson : function() {
				...
				if ($chk(this.options.cardPOR)) {// 9
					reqExecAPDU.ExecAPDUsReqest.CardPOR = this.options.cardPOR
							.toJson();
				} else {
...
JIM.CardDriver.CardPOR = new Class({
	...
	isSuccess : function() {
		var sw = this.getSw();
		var isSuccess = (this.successStatusCode == sw)
				|| (this.lockedSuccessStatusCode == sw);
		return isSuccess;
	},
