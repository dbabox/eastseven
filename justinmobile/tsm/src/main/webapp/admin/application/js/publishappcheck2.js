...
Application.Page = new Class({
	...
	getTestFileList : function(){
		...
		grid.inject(box2.getElement("[id='grid']"));
		
		var paging = new JIM.UI.Paging({
			url : url,
			limit : 10,
			head : {el: box2.getElement("[id='nextpage']"), showNumber: true, showText : false},
			onAfterLoad : function(data) {
				grid.empty();
				data.result.forEach(function(testFile, index) {
					var operation = '';
					var detail    = '<a class="b"  style="float : none;"  href="' + ctx + '/html/testfile/?m=downFile&tfId=' + testFile.id + '"><span>下载</span></a>';
					operation = detail;
					grid.push([ 
					           {content : testFile.originalName, properties : {align : "center"}}, 
					           {content : testFile.comments, properties : {align : "center"}}, 
					           {content : testFile.uploadDate, properties : {align : "center"}}, 
					           {content : testFile.seqNum,  properties : {align : "center"}}, 
					           {content : operation, properties : {align : "center", width : "90px"}}]);
				});
				if(data.result.length == 0){
					box2.getElement("[id='tfInfo']").set('html','<span>无数据记录</span>')
				}else{
					box2.getElement("[id='tfInfo']").set('html','<span></span>')
				}
			}
		});
		paging.load();
	...
	getTestReport : function(){
		 var url = ctx + "/html/appverTest/?m=index&search_EQL_appVer.id=" + this.applicationVersionId;
			...
			grid.inject(box2.getElement("[id='testReport']"));
			
			var paging = new JIM.UI.Paging({
				url : url,
				limit : 10,
				head : {el: box2.getElement("[id='trNextPage']"), showNumber: true, showText : false},
				onAfterLoad : function(data) {
					grid.empty();
					data.result.forEach(function(testReport, index) {
						grid.push([ 
						           {content : testReport.testDate, properties : {align : "center",width:'120px'}}, 
						           {content : testReport.mobileNo, properties : {align : "center",width:'120px'}}, 
						           {content : testReport.modelType,  properties : {align : "center",width:'120px'}}, 
						           {content : testReport.seType, properties : {align : "center",width:'100px'}},
						           {content : testReport.cardBaseInfo_name, properties : {align : "center",width:'100px'}},
						           {content : testReport.result, properties : {align : "center",width:'100px'}},
						           {content : testReport.resultComment, properties : {align : "center",width:'100px'}},
						           {content : testReport.author, properties : {align : "center",width:'100px'}}
						           ]);
					});
					if(data.result.length == 0){
						box2.getElement("[id='trInfo']").set('html','<span>无数据记录</span>')
					}else{
						box2.getElement("[id='trInfo']").set('html','<span></span>')
					}
				}
			});
			paging.load();
	...
	showApplicationDetails : function() {
		var target = box2.getElement("[id='applicationDetails']").empty();
			...
			onSuccess : function(json) {
				if (json.success && json.message!='') {
					...
					for (var i=0;i<ids.length;i++){
						new Element('img').set('src',ctx + "/html/application/?m=getAppImg&appImgId=" + ids[i]).set('width','95')
						.set('height','140').inject(template.getElement("[title='应用截图']"));
					}
				}
			}
		}).get();
	...
	showDownloadOrder : function() {
		var target = box2.getElement("[id='showDownloadOrder']");
		target.empty();
	...
	showDeleteOrder : function() {
		var target = box2.getElement("[id='showDeleteOrder']");
		target.empty();
	...
	setDeleteOrders : function() {
		...
		var target = box2.getElement("[id='setDeleteOrder']");
		target.empty();
	...
	showInstallOrder : function() {
		var target = box2.getElement("[id='showInstallOrder']");
		target.empty();
...
Application.Client = new Class({
	...
	showCilent : function(client) {
		var target = box2.getElement("[id='client']");
...
Application.LoadFile = new Class({
	...
	showLoadFile : function(loadFile) {
		...
		var target 
		//= $('uploadedCapsInfo').getElement("tbody");
		= box2.getElement("[id='uploadedCapsInfo']") ;
...
//			var loadFileDiv = new Element("div").set("html", "1111"); // 封装基本信息和模块
...
Application.LoadModule = new Class({
	...
	showLoadModule : function(loadModule) {
		...
		var loadFile = box2.getElement("[id='"+loadFileDivId+"']");
...
Application.Applet = new Class({
	...
	showApplet : function(applet) {
		...
		var loadModule = box2.getElement("[id='"+loadLoadModuleDivId+"']");
